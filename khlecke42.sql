/*
Adatok bevitele excel-ből sql-be
-Az Excelbe történő exportálás megoldott (pl. Azure Data Studio)
-Az Excelből történő importálás problémás
-Egyik lehetőség, hogy az adatokat Excelből egy sql kóddá alakítjuk és azt a kódot futtatjuk sql szerveren
-Ehhez nyújt segítséget az ExcelTools (ExcelTools.xlam)
-Csatolt fájlból ExcelTools.zip vagy
-Letöltés: github.com/farkas-mate/Excel
-Hely: C:\Users\<Felhasználó>\AppData\Roaming\Microsoft\Excel\XLStart
*/

-- a generált kódban az N' a UNIKódolást jelenti (ékezetek)
-- táblakontruktor lekérdezés
SELECT list.* 
FROM (
VALUES
...
select * from #list
order by ProductSubCategoryID -- string számok szerint

select * from #list
order by convert(int,ProductSubCategoryID)